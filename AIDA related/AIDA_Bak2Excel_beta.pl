...
#argument： 1. ARGV[0] casename
#					   用例名称-> 将以此命名DLF文件存放的根目录名称，如果没有参数输入，将会使用默认名称 DefaultTestCaseName+时间标志
#           2. ARGV[1] bool_isDecodewithBlueShark （Not Intergrated）
#					   是否使用BlueShark对文件进行Decode的判决条件
#************************************************************************************************
#	2012.11.02 sunyuting 将BlueShark部分加入判断条件，默认为使用
...
print "\nAll BAK Files have been renamed to DLF file and sorted into ".$dir_main."\n\n";
...
if ($ifDecodeWithBlueShark != 1)#判决是否使用blueshark
{
	...
	#@lines = $Blueshark -> waitfor ('Match'=>"/INIT/");
	if (@lines == 1)
	{
...
{
	print "BlueShark Decoding Passed\n";
}
